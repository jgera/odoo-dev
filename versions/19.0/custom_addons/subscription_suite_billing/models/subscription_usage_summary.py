...
class SubscriptionUsageSummary(models.Model):
    ...
    invoice_line_id = fields.Many2one('account.move.line', string='Invoice Line', readonly=True, copy=False, index=True)
    event_ids = fields.One2many('subscription.usage.event', 'summary_id', string='Usage Events', readonly=True)
    currency_id = fields.Many2one('res.currency', related='subscription_id.currency_id', store=True, index=True)
