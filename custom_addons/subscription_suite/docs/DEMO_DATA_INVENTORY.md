...
| Churn reason summaries | Generated | Generate from **Subscriptions -> Reporting -> Generate Churn Reasons** for cancelled/expired subscriptions in a selected period; static XML is intentionally avoided |
...
| Phase 6 | MRR snapshots, reconciliation rows, movement anomalies, KPI summaries, KPI dashboards, waterfalls, retention cohorts, revenue forecasts, ARPU summaries, LTV summaries, churn reason summaries, analytics smoke coverage, and generated-record lifecycle checks are generated from current or controlled records; later add multi-month MRR movement history and richer churn feedback classification |
