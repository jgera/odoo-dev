...
| Deferred revenue schedules | Generated | Generate from posted subscription invoices with `subscription_period_start` and `subscription_period_end`; static schedule XML is intentionally avoided so schedules reconcile to the generated invoice amount; blocked schedules expose missing period/configuration issues, recognition setup is company-scoped, and mixed invoices should recognize only recurring subscription service lines |
| Revenue recognition preview | Generated/Transient | Open **Subscriptions -> Billing -> Preview Recognition** after generating ready deferred revenue schedules; preview rows are transient and do not create journal entries or change recognition line states |
| Revenue recognition posting | Generated accounting entries | Open **Subscriptions -> Billing -> Post Recognition** after generating ready deferred revenue schedules; posting creates one journal entry per selected schedule and marks included due recognition lines recognized |
| Deferred revenue credit-note adjustments | Generated | Create posted subscription credit notes linked to original subscription invoices, then use **Adjust Credit Notes** or **Adjust Deferred Revenue**; adjustments are generated from refunds and may cancel/reduce draft lines or post reversal entries for already recognized revenue |
| Deferred revenue reconciliation | Generated | Generate from **Subscriptions -> Billing -> Generate Deferred Revenue Reconciliation** after creating posted subscription invoices, deferred revenue schedules, optional recognition journal entries, and optional credit-note adjustments; static XML is intentionally avoided |
...
| Performance scale smoke | Generated/Test-only | Covered by Phase 8 medium-scale automated tests for billing cron, dunning cron, recognition preview selection, and generated analytics idempotency; no static XML is loaded, and the full 10K generator remains deferred |
...
| Phase 7 | Deferred revenue schedules are generated from posted subscription invoices with valid service periods and required company recognition configuration; ready/recognized schedules are locked for audit, mixed invoices should exclude one-time lines, recognition preview is transient/no-posting, manual journal posting creates one posted move per schedule, credit-note adjustments are generated from posted refunds, reconciliation rows are generated from invoices/schedules/journal entries/adjustments, scheduled recognition posting creates generated run logs only after finance enables the company config gate, and accounting users get read-only finance visibility |
| Phase 8 | Security/access audit coverage for role and multi-company boundaries, medium-scale performance smoke data generated by tests/helpers, then complete end-to-end demo story and optional large-data generator |
